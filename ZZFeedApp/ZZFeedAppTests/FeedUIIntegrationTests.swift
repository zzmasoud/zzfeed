...
class FeedUIIntegrationTests: XCTestCase {
    ...
    func test_feedImageView_doesNotLoadImageAgainUntilPreviousRequestCompletes() {
        ...
        sut.simulateFeedImageViewVisible(at: 0)
        XCTAssertEqual(loader.loadedImageURLs, [image.imageURL], "Expected no request until previous request completes")
        ...
        sut.simulateFeedImageViewNotNearVisible(at: 0)
        sut.simulateFeedImageViewVisible(at: 0)
        XCTAssertEqual(loader.loadedImageURLs, [image.imageURL, image.imageURL, image.imageURL], "Expected third request when visible after cancelling previous complete")
        
        sut.simulateLoadMoreFeedAction()
        loader.completeLoadMore(with: [image, makeImage()])
        sut.simulateFeedImageViewVisible(at: 0)
        XCTAssertEqual(loader.loadedImageURLs, [image.imageURL, image.imageURL, image.imageURL], "Expected no request until previous request completes")
    }
